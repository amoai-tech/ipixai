...
interface QAFindingsPanelProps {
  ...
  shootId?: string;
  channels?: string[];
  initialResult?: QAAssetResult;
}

export function QAFindingsPanel({ assetId, shootId, channels, initialResult }: QAFindingsPanelProps) {
  ...
  const runQA = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/assets/${assetId}/qa`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ shootId, channels }),
      });
      if (!res.ok) {
      ...
      setError(err instanceof Error ? err.message : "Failed to run QA check");
    } finally {
      setLoading(false);
    }
  };
