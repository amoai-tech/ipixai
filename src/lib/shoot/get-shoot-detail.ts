...
const shootDetailSchema = z.object({
  shoot: z.object({
    ...
    approval_revision: z.number().int().positive().nullable().optional(),
    approval_plan_hash: z.string().nullable().optional(),
    approved_plan: z.unknown().nullable().optional(),
    planned_shoot_type: z.string().nullable().optional(),
  }),
  brand: z.object({
    id: z.string(),
    name: z.string(),
  ...
  assets: z.array(
    z.object({
      ...
      // approval. Optional so the reader tolerates a deployed
      // get_shoot_detail that predates the IPI-1119 migration (the migration
      // and this code deploy independently). `version` is bigint in Postgres;
      // accept number or string so the contract is robust to either encoding.
