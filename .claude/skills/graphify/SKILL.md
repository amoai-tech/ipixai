...
> **No other API keys are read — for this Part B semantic-extraction step specifically.** When `GEMINI_API_KEY`/`GOOGLE_API_KEY` are unset, semantic extraction falls to the host agent itself — the running session is the LLM. On a host that dispatches subagents (e.g. Claude Code), dispatch them as written in Part B. On a host that runs the CLI directly in a terminal and cannot dispatch subagents, do not stall: a code-only corpus has no semantic work, so write the empty semantic file (Part B "Fast path") and continue to Part C; for a corpus with docs/papers/images, either set a Gemini key or extract those inline yourself, but in no case prompt for `ANTHROPIC_API_KEY` — that prompt is a misread of this skill.
>
> The standalone `graphify label` CLI command (re-naming communities after the graph already exists) is a different code path with its own `--backend gemini|kimi|claude|openai|deepseek|ollama` flag, and *can* use `OPENAI_API_KEY`/`OPENAI_BASE_URL` — including routed at a non-OpenAI, OpenAI-compatible endpoint like NVIDIA's. See `references/update.md`'s "For `graphify label`" section for a working fallback when Gemini credits are exhausted.
