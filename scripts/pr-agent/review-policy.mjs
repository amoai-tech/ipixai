...
export const CERT_HISTORY_MARKER = "<!-- ipix-pr-agent-cert-history -->";
...
export function certificationMarker({ baseSha, headSha }) {
  ...
  return `<!-- ipix-pr-agent-cert base=${baseSha.toLowerCase()} head=${headSha.toLowerCase()} -->`;
}

export function hasCertificationForBase(body, baseSha) {
  assertSha(baseSha, "baseSha");
  return (body ?? "").toLowerCase().includes(`<!-- ipix-pr-agent-cert base=${baseSha.toLowerCase()} head=`);
}
...
export function selectReviewCommand({ action, certificationBodies, baseSha }) {
  ...
  return certificationBodies.some((body) => hasCertificationForBase(body, baseSha)) ? "/review -i" : "/review";
}
...
export function verifyReviewResult({ comments, startedAt, reviewCommand, baseSha }) {
  ...
  const fresh = comments.some((comment) => {
    ...
    const standalone = body.includes("## Standalone PR Review") &&
      ...
      body.includes("## iPix PR Review");
    const incrementalSkipped = reviewCommand === "/review -i" &&
      priorSameBaseCertification && isIncrementalSkipNotice(body);
