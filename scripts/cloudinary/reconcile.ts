...
function requireRuntimeConfig(): RuntimeConfig {
  ...
  if (!config) throw new Error("reconcile_runtime_config_unavailable");
  return config;
}
...
function sameProviderIdentityVersion(a: ProviderAsset, b: ProviderAsset): boolean {
  return a.assetId === b.assetId && a.version === b.version;
}

type DbPage<T> = { data: T[] | null; error: { message: string } | null };

export async function collectSupabasePages<T>(
  readPage: (from: number, to: number) => PromiseLike<DbPage<T>>,
  failureCode: string,
): Promise<T[]> {
  const rows: T[] = [];
  for (let from = 0; ; from += SUPABASE_PAGE_SIZE) {
    const { data, error } = await readPage(from, from + SUPABASE_PAGE_SIZE - 1);
    if (error || !data) throw new Error(`${failureCode}:${error?.message ?? "empty"}`);
    rows.push(...data);
...
export async function collectActiveV2Pages(
  ...
  do {
    ...
    if (nextCursor) {
      if (seenCursors.has(nextCursor)) throw new Error("cloudinary_search_repeated_cursor");
...
export async function listDbMirrorsFromSupabase(): Promise<DbMirror[]> {
  ...
  const rows = await collectSupabasePages<DbRow>((from, to) =>
    supabase
      .from("cloudinary_assets")
      .select("asset_id, cloudinary_asset_id, version, status, metadata")
      .order("cloudinary_asset_id", { ascending: true, nullsFirst: true })
      .order("asset_id", { ascending: true })
      .range(from, to),
    "cloudinary_assets_read_failed",
  );
  ...
  for (const assetIds of batches(candidateIds, 100)) {
    events.push(...await collectSupabasePages<EventRow>((from, to) =>
      supabase
        .from("asset_events")
        .select("cloudinary_asset_id, version, kind, metadata")
        .in("cloudinary_asset_id", assetIds)
        .order("cloudinary_asset_id", { ascending: true })
        .order("version", { ascending: true, nullsFirst: true })
        .range(from, to),
      "asset_events_read_failed",
    ));
  }

  return buildDbMirrors(rows, events);
}

export function buildDbMirrors(rows: DbRow[], events: EventRow[]): DbMirror[] {
  return rows.map((row) => {
...
export async function runReconciliation(deps: ReconcileDependencies): Promise<ReconcileReport> {
  // Read the authoritative DB candidate set before any provider request.
  const dbMirrors = await deps.listDbMirrors();
  const activeProviderAssets = await deps.listActiveV2();
  ...
  for (const lookup of lookedUpProviderAssets) {
    const matches = providerAssets
      .map((active, index) => ({ active, index }))
      .filter(({ active }) => active.assetId === lookup.assetId);
    if (matches.length === 1 && sameProviderIdentityVersion(matches[0]!.active, lookup)) {
      providerAssets[matches[0]!.index] = lookup;
      continue;
    }
...
export async function main(deps?: ReconcileDependencies): Promise<number> {
  // Do this before runReconciliation starts either provider/database request.
  ...
  const report = await runReconciliation(resolvedDeps);
