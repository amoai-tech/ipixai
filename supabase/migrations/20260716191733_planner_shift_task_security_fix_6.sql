-- IPI-649 · PLN-DATA-001B-M — sixth security-fix pass, from continued
-- Bugbot/Sentry re-review of commit ddc1f290 (round 4) on PR #418.
--
-- Fixed:
-- 1. JSON null dates clear schedule (planner_shift_task, HIGH — Sentry +
--    Cursor, same finding): round 4 only checked KEY PRESENCE
--    (`v_task ? 'newStartDate'`), not that the VALUE isn't a JSON `null`.
--    `{"newStartDate": null, ...}` passes the presence check, then
--    `v_task ->> 'newStartDate'` on a JSON null still returns SQL NULL,
--    reopening the exact "omitted date clears the schedule" bug round 4
--    was meant to close — just via `null` instead of an absent key. Fixed:
--    the check now also rejects `jsonb_typeof(...) = 'null'` for both date
--    keys.
-- 2. Missing taskId not rejected (planner_shift_task, MEDIUM — Cursor): the
--    p_changed_tasks pre-pass cast `(v_task ->> 'taskId')::uuid` without
--    checking the result — a missing or JSON-null `taskId` casts cleanly to
--    SQL NULL (no data_exception), so it passed through as v_task_id = NULL.
--    NULL never matches `= any(v_seen_ids)`, so repeated NULL-taskId entries
--    were never caught as duplicates, and downstream queries scoped to a
--    NULL taskId silently matched nothing rather than raising a clear
--    error. Fixed: v_task_id is null is checked immediately after the cast
...
begin
  ...
  -- Reject a missing/null taskId, a duplicate taskId, or an entry with
  -- either date key absent OR present-but-JSON-null — rather than silently
  -- double-processing, matching nothing, or NULL-clearing a schedule.
  ...
  loop
    ...
    if not (v_task ? 'newStartDate' and v_task ? 'newEndDate')
      or jsonb_typeof(v_task -> 'newStartDate') = 'null'
      or jsonb_typeof(v_task -> 'newEndDate') = 'null'
    then
      return jsonb_build_object('ok', false, 'code', 'INVALID_INPUT');
    end if;
  end loop;
...
comment on function public.planner_shift_task(uuid, uuid, integer, text, jsonb, jsonb) is
  'IPI-649 (security-fix pass 6) — persists a schedule shift PlannerEngine.shiftTask() already computed in-memory. NULL-safe idempotency key/delta, malformed-cast-safe (data_exception -> INVALID_INPUT), duplicate-taskId-safe and null-taskId-safe, requires both newStartDate/newEndDate present and non-null on every changed-task entry, request hash includes root task id, replay exempted from the terminal-instance guard, no audit event for a fully no-op shift. See pass 1''s comment for the full locking/authorization/dependency-comparison model.';
