-- IPI-367 follow-up fix · found by automated PR review: the cross-org
-- company FOUND check only ran inside the p_decision = 'won' branch, but
-- crm_activities uses v_company_id for BOTH decisions — a 'lost' conversion
-- on a deal with a cross-org company_id still wrote a data-isolation-
-- violating activity row. Moved the check to run unconditionally.
