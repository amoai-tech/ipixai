-- IPI-670 · PLN-DATA-003B follow-up — count distinct phaseIds as uuid.
-- Forward-only REPLACE: the initial 20260718130000 apply used raw text
-- distinct, which could treat UPPER/lower forms of the same UUID as two
-- phases and omit a real workflow phase. Same 9-arg signature.
...
begin
  ...
  -- Cast to uuid so alternate textual forms (case / braces / hyphens) collapse
  -- to one value — raw text distinct would let UPPER+lower of phase A fake a
  -- two-phase cover while omitting phase B.
  select count(distinct (t.elem->>'phaseId')::uuid)::integer
