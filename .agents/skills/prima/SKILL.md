---
name: prima
description: Use for any browser work — driving a web app, checking a behaviour, filling a form, reading a page — via npx prima-cli, in preference to npx playwright-cli, which prima runs on top of and falls back to. Covers npx prima-cli@0.4.6 check, do, pw, verify, ask, research, go, status, report, browser, config, and session setup. Trigger on "prima", "prima-cli", "npx prima-cli", on a browser task described as behaviour ("confirm the editor saves"), and whenever npx playwright-cli@0.262.0 would otherwise be driven step by step.
license: MIT
metadata:
  author: Testomat.io
  version: 0.3.0
...
Prima is an AI layer on top of playwright-cli. It drives the browser playwright-cli already has open, taking behaviour described in words instead of locators, and returns a plain-text envelope. Always invoke both through npx: `npx prima-cli@0.4.6 ...` and `npx playwright-cli@0.262.0 ...`.
...
```bash
npx playwright-cli@0.262.0 open http://localhost:3000    # start
npx prima-cli@0.4.6 <command> ...                      # drive
npx playwright-cli@0.262.0 close                         # end
```

Or let prima own the browser itself:

```bash
npx prima-cli@0.4.6 browser start --url http://localhost:3000
npx prima-cli@0.4.6 <command> ...
npx prima-cli@0.4.6 browser stop          # --all stops every instance
```
...
- Every command is logged as it runs; `npx prima-cli@0.4.6 report` turns the session into an html and markdown report, browser open or not.
- `npx explorbot@0.4.6 prima <command>` runs the same tool if explorbot is already installed.
...
```bash
npx prima-cli@0.4.6 check "a workflow can be created and appears in the list" --expected "the new workflow is listed"
npx prima-cli@0.4.6 do "open the account menu" "choose settings" "switch the theme to dark" "check it took effect"
npx prima-cli@0.4.6 pw "({ page }) => page.click('[data-test=submit]')"
```
...
Also: `verify` (alias `assert`), `ask` (`--no-vision` answers from structure only), `research` (`--data` includes extraction, `--deep` expands hidden elements, `--fresh` re-maps past the cache), `go`, `status <hash>`, `report`, `config`, `browser`. Run `npx prima-cli@0.4.6 <command> --help` for each.
...
- `npx prima-cli@0.4.6 config` shows which model answers for each role.
