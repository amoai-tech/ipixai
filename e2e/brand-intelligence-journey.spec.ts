 ...
 * status and draft in the target Supabase project (each test then rejects the
 * draft it produced, never approves it), and the server needs the
...
/**
 * A run cannot start while a draft is waiting for review (the workflow's
 * claim guard rejects it), so a leftover draft would let a test "pass" on
 * someone else's result. Require a clean brand before starting.
 */
const requireNoDraft = async (page: Page) => {
  await page.goto(`/app/brands/${brandId}`);
  await expect(
    page.getByText(/— Brand DNA draft$/),
    "the QA brand must have no pending draft before a run (reject it first)",
  ).toHaveCount(0);
};

/** Rejects the draft this test just produced, so the next run starts clean. Never approves. */
const rejectOwnDraft = async (page: Page) => {
  await page.getByRole("button", { name: "Reject", exact: true }).click();
  await expect(page.getByText(/— Brand DNA draft$/)).toHaveCount(0, { timeout: NAV_TIMEOUT_MS });
};
...
test.describe("brand intelligence journey (authenticated) @S3c7e1290", () => {
  ...
  test("started from Planner chat, the analysis reaches the Brand DNA draft", async ({ page }) => {
    test.setTimeout(ANALYSIS_TIMEOUT_MS + NAV_TIMEOUT_MS * 6);
    const problems = collectBrowserProblems(page);
